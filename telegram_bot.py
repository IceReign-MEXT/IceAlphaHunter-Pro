...
💰 Auto-trade: {'ON' if config.AUTO_TRADE_ENABLED else 'OFF'}
    ...
    def cmd_help(self, update: Update, context: CallbackContext):
        ...
        if update.effective_user.id != config.ADMIN_ID:
            return
        update.message.reply_text("📚 Commands: /status /stats /trades /balance /panic /stopbot", parse_mode='Markdown')
...
Auto-Trade: {'✅ ON' if config.AUTO_TRADE_ENABLED else '❌ OFF'}
    ...
    def cmd_panic_sell(self, update: Update, context: CallbackContext):
        ...
        for trade in trades:
            try:
                ...
                if result.success:
                    db.close_trade(trade.get('id'), result.output_amount, 0, 0, result.signature or '')
                    sold += 1
    ...
    def cmd_broadcast(self, update: Update, context: CallbackContext):
        ...
        if not message:
            ...
            return
        try:
            self.bot.send_message(chat_id=config.CHANNEL_ID, text=f"📢 **Admin**\n\n{message}", parse_mode='Markdown')
    ...
    async def _notify_channel(self, whale, result, position):
        ...
        try:
            message = f"""
🐋 **WHALE FOLLOWED**
Whale: `{whale.trader_address[:8]}...`
Token: {whale.token_symbol}
Invested: {position:.3f} SOL
Got: {result.output_amount:.4f} {whale.token_symbol}
            """
            self.bot.send_message(chat_id=config.CHANNEL_ID, text=message, parse_mode='Markdown')
        except Exception as e:
